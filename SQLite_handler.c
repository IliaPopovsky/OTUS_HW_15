...
int print_result(void * args, int colCount, char **columns, char **colNames){
    if(args){};                    //To remove warning`а -Wunused-but-set-parameter
...
int main(int argc, char * argv[]){
    if(argc != 4){
        printf("Use ./SQLite_handler db_name, table_name, column_name\n");
        return(EXIT_FAILURE);
    }
    sqlite3 *db;
    int res;
    ...
    snprintf(sql, sizeof(sql), "SELECT AVG(%s) as AVG, max(%s) as MAX, min(%s) as MIN, (AVG(%s*%s) - AVG(%s)*AVG(%s)) as DEVIATION, sum(%s) as SUM from %s;", 
    column_name, column_name, column_name, column_name, column_name, column_name, column_name, column_name, table_name);
